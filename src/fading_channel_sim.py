...
class FadingSimulation:
    def __init__(self, num_samples, fs, K, N, h, Doppler_compensate):
    ...
    def rician_fading_accurate(self, fd):
        ...
        Z *= np.sqrt(1 / ((self.N + 1) * (1 + self.K)))
        # LOS component
        theta_0 = np.pi / 4
        phi_0 = 2 * np.pi * np.random.rand() - np.pi       # phi_{0, k}
        Z_LOS = np.sqrt(self.K / (1 + self.K)) * np.exp(1j * (omega_m * t * np.cos(theta_0) + phi_0))
    ...
    def run_simulation(self, theta_degrees, fc_array, distance_satellite):
        ...
        for fc in fc_array:
            for theta_deg, theta_rad in zip(theta_degrees, theta_radians):
                ...
                lower_doppler_rayleigh_fading = self.rician_fading_accurate(fd / 100)
    ...
    def channel_with_UE_movement(self, theta_degrees, fc_array, UE_speed, UE_moving_direction,distance_satellite):
        ...
        for fc in fc_array:
            for theta_deg, theta_rad in zip(theta_degrees, theta_radians):
                ...
                # Generate Rayleigh fading sequence which is temporally correlated
                rayleigh_fading_samples = self.rician_fading_accurate(fd_due_to_UE_movement)
                ...
                # Generate temporally correlated Rayleigh time series with lower Doppler shift
                lower_doppler_rayleigh_fading = self.rician_fading_accurate(fd_due_to_UE_movement / 100)
